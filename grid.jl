...
function grid_from_mesh(mesh::MeshData, config; grid_type=CG, order=1, mixed=false)
    ...
    if Gness == 1 # CG
        ...
        t_nodes1 = Base.Libc.time();
        (allnodes, loc2glb) = remove_duplicate_nodes(config, tmpallnodes, loc2glb, tol=tol);
        t_nodes1 = Base.Libc.time() - t_nodes1;
    ...
    if mixed # element types
        return (refels, Grid(config.float_type, allnodes, bdry, bdryfc, bdrynorm, bids, node_bids, loc2glb, glbvertex, f2glb, 
                        element2face, face2element, facenormals, faceRefelInd, facebid, true, etypes, refel_ind));
    else # one element type
        return (refel, Grid(config.float_type, allnodes, bdry, bdryfc, bdrynorm, bids, node_bids, loc2glb, glbvertex, f2glb, 
...
function remove_duplicate_nodes(config::FinchConfig, nodes::Matrix, loc2glb::Matrix; tol=1e-12, scale=1, depth=5, mincount=50, other2glb=[])
    # defaults
