...
function build_geometric_factors(config, refel, grid::Grid; do_face_detj::Bool=true, 
                                do_vol_area::Bool=false, constant_jacobian::Bool=false)
    ftype = config.float_type;
    ...
    for e=1:nel
        ...
        (e_detJ, e_J) = geometric_factors(config, refeli, xe, constantJ=constant_jacobian);
...
function geometric_factors(config, refel::Refel, pts::Matrix; constantJ::Bool=false, do_J::Bool=true)
