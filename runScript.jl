...
function profileSerialLinearSystemSolve( fileVal, configObj, orderVal = 1, filenamePrefix = "SerialLinearSystemScaling" )
...
function gpuMATVECGridSpeedupScaling( gmshFolderName, configObj, figname )
...
function gpuMATVECGridFlopScaling( gmshFolderName, configObj, figname )
    ...
    lvlVals = zeros( length( meshVals ) )
    perfVals = zeros( length( meshVals ) )
    numNodeVals = zeros( length( meshVals ) )
    ...
    for (meshIdx, meshVal) in enumerate( meshVals )
        ...
        lvlVals[ lvlVal ] = lvlVal
        numNodeVals[ lvlVal ] = getNumNodes( meshVal )
    ...
    for tagVal in tagVals
        ...
        for (lvlVal, benchVal) in gpuMatvecBenchGroups
            ...
            gpuMedianVal = median(benchVal[ tagVal ]);
            gpuMedianTimeVal = (gpuMedianVal.time) * ( 10 ^ ( -6 ) );

            perfVals[lvlVal] = numNodeVals[ lvlVal ] / gpuMedianTimeVal;
    ...
    legend()
    xlabel("Levels")
    ylabel("Performance = NumNodes/Time")
    savefig( figname );

end

function gpuMATVECGridFlopScaling2( gmshFolderName, configObj, figname )
    ...
    for tagVal in tagVals
        ...
        println(gpuMatvecBenchGroups)
        for (lvlVal, benchVal) in gpuMatvecBenchGroups

            gpuMedianVal = median(benchVal[ tagVal ]);
            gpuMedianTimeVal = (gpuMedianVal.time) * ( 10 ^ ( -6 ) );

            # println(gpuMedianTimeVal)
            perfVals[lvlVal] = numNodeVals[ lvlVal ] / gpuMedianTimeVal;
